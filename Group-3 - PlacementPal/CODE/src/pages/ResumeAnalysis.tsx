...
const SkillPill = ({ skill, matched }: { skill: string; matched: boolean }) => (
  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-mono border ${matched
      ...
      <AnimatePresence initial={false}>
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-[82%] rounded-xl px-3 py-2 text-xs leading-relaxed ${msg.role === 'user'
...
const ResumeAnalysisPage = () => {
  ...
  const handleAnalyzeResume = async () => {
    ...
    if (!session?.access_token) {
      toast({ title: 'Not logged in', description: 'Please log in to analyze your resume.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      // Send extracted text to new NLP backend
      const { data, error: invokeError } = await supabase.functions.invoke('resume-analyze', {
        body: {
          resumeText: fullText,
          domain: selectedDomain ?? 'Overall'
        }
      });
      ...
      console.log('Analysis object type:', typeof data.analysis);
      console.log('Analysis object:', data.analysis);

      const a = data.analysis;
      // Normalise to the shape chat hook expects
      ...
      await supabase.from('resumes').upsert({
        ...
        analysis_json: JSON.parse(JSON.stringify(normalized)),
          ...
          {(() => {
            ...
            return (
              <motion.button
                ...
                className={`w-full p-4 rounded-lg border-2 text-left transition-all duration-300 ${active ? 'border-primary bg-primary/10' : 'border-border bg-muted/30 hover:border-primary/40'
                  }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${active ? 'bg-primary/20 border border-primary/50' : 'bg-muted border border-border'
                    }`}>
            ...
            {DOMAINS.filter(d => d.id !== 'Overall').map((d) => {
              ...
              return (
                <motion.button
                  ...
                  className={`p-4 rounded-lg border-2 text-left transition-all duration-300 ${active
                    ? `border-${d.color} bg-${d.color}/10`
                    : 'border-border bg-muted/30 hover:border-primary/40'
                    }`}
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${active ? `bg-${d.color}/20 border border-${d.color}/50` : 'bg-muted border border-border'
                      }`}>
  ...
  const ResumeResults = () => {
    ...
    return (
      <div className="space-y-5">
        ...
        <CyberButton variant="ghost" size="sm" className="w-full min-w-[120px]" onClick={handleRemoveFile}>
        ...
        <AnimatePresence>
          {selectedDomain && (
            ...
            >
              ...
              <CyberCard variant="glow" className="h-[650px]">
                ...
                <div className="flex gap-2 mb-5">
                  {(['resume', 'linkedin'] as AnalysisMode[]).map(m => (
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-display font-semibold text-sm transition-all ${analysisMode === m
                    ...
                    <motion.div key="resume" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      ...
                      {!resumeFile ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <CyberButton variant="primary" className="w-full min-w-[120px]" onClick={handleAnalyzeResume} disabled={isAnalyzing} glowing>
                            {isAnalyzing
                    ...
                    <motion.div key="linkedin" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      {!linkedinAnalysis ? (
                        <div className="space-y-4">
                          ...
                          <CyberButton variant="primary" className="w-full min-w-[120px]" onClick={handleAnalyzeLinkedin} disabled={isAnalyzingLinkedin || (!linkedinUrl.trim() && !linkedinProfileText.trim())} glowing>
                        ...
                        <div className="space-y-4">
                          ...
                          <CyberButton variant="ghost" size="sm" className="w-full min-w-[120px]" onClick={() => { setLinkedinUrl(''); setLinkedinProfileText(''); setLinkedinAnalysis(null); }}>
              ...
              <CyberCard variant="accent" className="h-[650px]">
                <div className="h-full flex flex-col min-h-0">
                {/* Header */}
